...
var ajaxifyShopify = (function(module, $) {
  ...
  flipSetup = function () {
    ...
    var checkoutBtn = $('<a href="/cart" class="flip-back" style="background-color: #C00; color: #fff;" id="flip-checkout">' + "PROCEED TO SECURE CHECKOUT" + '</a>').addClass($btnClass),
        flipLoader = $('<span class="ajaxifyCart-loader"></span>'),
        flipExtra = $('<div class="flip-extra">' + "Translation missing: en.cart.general.or" + ' <a href="#" class="flip-cart">' + "Translation missing: en.cart.general.view_cart" + ' (<span></span>)</a></div>');
  ...
  modalSetup = function () {
    ...
    $modalContainer.prepend('<button class="ajaxifyCart--close" title="' + "Translation missing: en.cart.general.close_cart" + '">' + "Translation missing: en.cart.general.close_cart" + '</button>');
  ...
  buildCart = function (cart) {
    ...
    // Show empty cart
    if (cart.item_count === 0) {
      $cartContainer.append('<h2>' + "Translation missing: en.cart.general.empty" + '</h2><span>' + "Translation missing: en.cart.general.continue_browsing_html" + '</span>');
  ...
  adjustCart = function () {
    ...
    $body.on('submit', 'form.cart-form', function(evt) {
       if(!jQuery.isEmptyObject(cart_array)){
        // alert("You can not add PREORDER product with non-preorder products");
          // evt.preventDefault();                   
       }
      if (isUpdating) {
        evt.preventDefault();
