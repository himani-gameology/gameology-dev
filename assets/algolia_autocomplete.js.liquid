(function(algolia) {
  ...
  client.addAlgoliaAgent('Shopify Integration');
  ...
  return function multiQuerySource(singleIndex, params, extendHits = null) {
      return debouncePromise(function search(query, cb) {
        const index =
        ...
        .then((res) => {
          ...
          .catch((err) => cb([], err));
      }, 1200); // Zeta Digital - Debounce Seconds Setting
    };
  }
  ...
  var config = {
    ...
    products_query_suggestions: {
      ...
      hitsPerPage: 6, // number of hits to display
      index: index('shopify_products_query_suggestions'), // the index must have the same prefix of the shop
      templates: {
  ...
  // Add event listeners for autocomplete open and close
  autocompleteInstance.on('autocomplete:shown', function() {
    toggleBodyScroll(true);
  });
  
  autocompleteInstance.on('autocomplete:closed', function() {
